...
prm =  CalcOperatorPrm_kato(outsideTemperature=28,max_time=max_time,i_max=2,i_min=0,heatTransferCoef_water=270,tau=30,p=0.15,p2=0.15,p_A=0.7,...
...
%測定ノイズ(100Hzサンプリング)
tubeNoise = 1*1.5*0.15*randn(3,100*length(variable.tubeGairan));
almiNoise = 1*0.15*randn(3,100*length(variable.tubeGairan));

% variable.tubeGairan([1,3],400:end) = -0.5;
variable.tubeGairan([1,3],800:end) = 1*-1;

variable.almiGairan([1,3],450:end) = 1*-0.5;
...
for cycleCount = 1:length(t)
    if cycleCount > 0 
        ...
        % 外乱除去制御系
        if cycleCount >1
            [variable.f_1(:,cycleCount), variable.f_2(:,cycleCount), operatorTempVariable.disturbanceRejectionOperator] = disturbanceRejectionOperator_prev(cycleCount,dt,...
...
TEMPERATURE_GRAPH_TITLE = ["temperature_A_I_F_prev"];
...
CONTROLINPUT_GRAPH_TITLE = ["currrent_A_I_F_prev"];
